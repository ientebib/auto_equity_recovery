...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:3000", "http://localhost:3002", "http://localhost:3004", "http://localhost:3005"],  # Support all common dev ports
...
def get_latest_analysis_file(campaign_name: str) -> Optional[str]:
    ...
    timestamp_dirs = [d for d in os.listdir(campaign_path) 
                     if os.path.isdir(os.path.join(campaign_path, d)) and 
                     d.startswith('2025-')]
    ...
    analysis_files = glob.glob(os.path.join(campaign_path, latest_dir, "*analysis*.csv"))
...
def process_lead_data(df: pd.DataFrame, campaign_id: str) -> List[dict]:
    ...
    string_columns = ['name', 'last_name', 'clean_email', 'summary', 'suggested_message_es', 
                     'primary_stall_reason_code', 'next_action_code', 'transfer_context_analysis',
                     'cleaned_phone', 'lead_created_at', 'last_user_message_text', 'last_kuna_message_text']
    for col in string_columns:
        if col in df.columns:
            df[col] = df[col].fillna('').astype(str)
    
    # Handle boolean columns
    boolean_columns = ['NO_USER_MESSAGES_EXIST', 'IS_WITHIN_REACTIVATION_WINDOW', 
                      'IS_RECOVERY_PHASE_ELIGIBLE', 'handoff_invitation_detected', 
                      'handoff_response', 'handoff_finalized', 'human_transfer']
    ...
    for _, row in df.iterrows():
        ...
        completion_status = cache.get_lead_completion_status(phone, campaign_id, conversation_digest)
        ...
        lead = {
            ...
            "avatar": f"/avatars/{first_name.lower()}.jpg" if first_name else "/avatars/default.jpg",
...
async def get_leads(
    ...
    analysis_file = get_latest_analysis_file(campaign_id)
    if not analysis_file:
        raise HTTPException(status_code=404, detail=f"No analysis data found for campaign: {campaign_id}")
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error processing leads data: {str(e)}")
...
async def mark_lead_complete(campaign_id: str, lead_id: str, request: CompleteLeadRequest):
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(status_code=500, detail="Failed to mark lead as completed")
            
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error marking lead complete: {str(e)}")

@app.get("/test-endpoint")
async def test_endpoint():
    """Test endpoint to verify server is running latest code"""
    return {"message": "Server is running latest code", "timestamp": datetime.now().isoformat()}
...
async def get_completion_stats(campaign_id: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error getting completion stats: {str(e)}")
...
async def get_completed_leads(campaign_id: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error getting completed leads: {str(e)}")
...
async def add_lead_note(lead_id: str, request: AddNoteRequest):
    ...
    return {"message": f"Note added to lead {lead_id}", "note": request.note, "author": request.author}
