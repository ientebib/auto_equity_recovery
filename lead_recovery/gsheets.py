...
def upload_to_google_sheets(
    csv_path: Path, sheet_id: str, worksheet_name: str, credentials_path: str = None
):
    ...
    logger.info(
        f"Attempting to upload {file_path.name} to Google Sheet ID {sheet_id}, worksheet '{worksheet_name}'"
    )
    ...
    if credentials_path is None:
        ...
        if credentials_path is None:
            raise ConfigurationError(
                "No Google credentials path provided. Set GOOGLE_CREDENTIALS_PATH in .env or GOOGLE_APPLICATION_CREDENTIALS environment variable."
            )
    ...
    try:
        ...
        if "PERMISSION_DENIED" in str(e):
            logger.error(
                "Ensure the service account email has edit access to the Google Sheet."
            )
        raise
    except Exception as e:
        logger.error(
            f"An unexpected error occurred during Google Sheets upload: {e}",
            exc_info=True,
        )
