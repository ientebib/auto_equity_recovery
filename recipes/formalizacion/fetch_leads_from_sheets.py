...
def main():
    ...
    # Date filtering
    df['parsed_date'] = df[date_col].apply(lambda x: parse(x, dayfirst=False) if isinstance(x, str) and x else pd.NaT)
    ...
    if not result_df.empty:
        print(f"   -> Sample phone numbers: {result_df['cleaned_phone'].head().tolist()}")
