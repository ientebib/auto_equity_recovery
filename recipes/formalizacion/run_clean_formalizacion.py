...
from recipes.formalizacion.run_recipe import main as run_standard_recipe
...
def generate_smart_summary(row):
    ...
    if pd.isna(row.get('documentos_enviados_analisis')) or row.get('documentos_enviados_analisis') == '':
    ...
    objection = str(row.get('objecion_principal_cliente', '')).lower()
...
def clean_output_data(df):
    ...
    print(f"   -> Eliminated {len(df.columns) - len(essential_columns)} unwanted columns")
    
    return clean_df


def upload_to_sheets(df, sheet_id="1nAU3lsPo98dTqaGOChhJM4WQKhyeItaqRGb5TvEFaXg", worksheet_name="Bot Live"):
...
async def main():
    ...
    timestamped_filename = output_dir / f"clean_formalizacion_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}.csv"
